...
static void MakeAABBWeightedUnion_SSEVectorized(benchmark::State & state)
{
    std::vector<Geometry::ShipAABB> aabbs;
    aabbs.reserve(SampleSize);
    for (size_t i = 0; i < SampleSize; ++i)
    ...
    for (auto _ : state)
    {
        result = Algorithms::MakeAABBWeightedUnion_SSEVectorized(aabbs);
    }

    benchmark::DoNotOptimize(result);
}
BENCHMARK(MakeAABBWeightedUnion_SSEVectorized);
#endif
