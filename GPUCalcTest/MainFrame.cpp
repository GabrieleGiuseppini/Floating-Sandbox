    ...
    : mMainApp(mainApp)
{
    ...
    //
    // Create Test GPUCalc context
    //

    GPUCalcContextFactory::GetInstance().RegisterOpenGLContextFactory(
        [this]() -> std::unique_ptr<IOpenGLContext>
        {
            return std::make_unique<OpenGLContext>();
        });

    mTestGPUCalcContext = GPUCalcContextFactory::GetInstance().CreateTestContext(10);
    ...
    mTestGPUCalcContext->Add(a, b, result);

    // TODOTEST
    mTestGPUCalcContext2 = GPUCalcContextFactory::GetInstance().CreateTestContext(10);
    mTestGPUCalcContext2->Add(a, b, result);
