...
void ViewManager::OnNewShip(Geometry::AABBSet const & allAABBs)
{
    if (mDoAutoZoomOnShipLoad)
    {
        ...
        {
            DoAutoFocus(allAABBs);
        }
        else
        {
            // Pointless, will be done right away
        }
    }
...
void ViewManager::AutoFocus(Geometry::AABBSet const & allAABBs)
{
    if (!mAutoFocus.has_value())
    {
        DoAutoFocus(allAABBs);
    ...
    {
        // Pointless, as we'll do it at the next Update()
    }
}
...
void ViewManager::DoAutoFocus(Geometry::AABBSet const & allAABBs)
{
    assert(mAutoFocus.has_value());
